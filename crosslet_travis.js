...
// method that we will use to update the control based on feature properties passed
info.update = function (props) {
    this._div.innerHTML = '<h1>Armutsquoten in Bern (2012)</h1>' +  
                           'Ein bisschen Text<br/>Ein bisschen Text<br/>Ein bisschen Text<br/>Ein bisschen Text<br/>';
};
...
// so I can use <br> inside the tooltip
      ...
      function mousemove(d){
          tooltip
              ...
              .style("top", d3.event.pageY + "px")
              .style("left", d3.event.pageX + "px")
              .html(d.properties.name + ": " + Math.round(d.properties.value*10000)/100 + "%<br \>" + 
                    "Anzahl Haushalte: " + getValue(d.properties.nHH,"zu wenig")
                    );  
      }
      ...
      function mouseout(){
       ...
       function getValue(value,message){
        if(value) {
          return value;
        } else {
          return message;
...
.attr("id", function(d) {
  ...
  MapView.prototype._renderMap = function(data, formatter, hoverFunc) {
    ...
    this.feature.attr("style", function(d) {
      ...
      id = d.properties[_this.config.map.geo.id_field];
      d.properties.value = data[id];
      if (_.isNumber(data[id])  && d.properties.value!="-") {
        return "fill: " + formatter(d.properties.value);
      } else {
        return "display:none";
      }
